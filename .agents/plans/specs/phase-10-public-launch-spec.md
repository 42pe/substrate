...
**Last updated:** 2026-09-06 (licensing addendum §3.7; D2's `0.5.0` superseded — see §3.7)
**Previously updated:** 2026-06-10
...
### 3.7 Licensing addendum (2026-09-06 — post-approval)

This spec was approved against an **MIT** repo. On 2026-09-06 the project relicensed to
**Apache-2.0**, with contributions accepted under a **DCO** sign-off and explicitly **no CLA**
(`decisions.md` → "Licensing (2026-09-06)"; commit `4aa8501`). The launch requirements change as
follows. Nothing in §3.1–§3.6 is invalidated — §3.4's sweep widens and two requirements are added.

**R-L1 — the public tree must assert exactly one license.** §3.2 (D1) publishes all of `.agents/`.
Six planning loci asserted MIT after the relicense. They are annotated *superseded* rather than
rewritten — the convention `decisions.md` already uses — because they are the record of what earlier
phases decided, and erasing MIT from them would falsify that record. §3.4's sweep therefore covers
**licensing terms repo-wide**, not just go-live phrasing in the five public-facing docs.

**R-L2 — the DCO must be enforced before trigger B.** `CONTRIBUTING.md` and `README.md` now *require*
`git commit -s`, and nothing checks it. A rule published without enforcement is worse than no rule: the
first outside contributor does the work, then gets bounced. Enforcement (a `pull_request` check scoped
to the **PR commit range** — history predating the DCO must not fail it — plus a PR template) is a
**launch requirement**, not a follow-up. If it cannot land, the correct move is to soften the
CONTRIBUTING wording to a request, not to publish an unapplied requirement.

**R-L3 — bundled third-party code must carry its notices.** `files` ships `dist/**`. `dist/server/**`
leaves dependencies external (npm resolves them at install — nothing redistributed). `dist/ui/**` is a
**`vite build` bundle** that embeds React, `marked`, `dompurify` and the Tailwind/ShadCN layer, whose
MIT/BSD terms require their copyright notices to travel with a redistribution. A generated
`THIRD-PARTY-NOTICES.md`, shipped in `files` and regenerated by the build, satisfies this.
**This predates the relicense** — it is not a consequence of moving to Apache-2.0. It becomes
*consequential* here because this phase produces the first tarball with an audience.

**No `NOTICE` file.** Apache-2.0 §4(d) binds a redistributor only when the upstream work carries one.
Substrate has none and vendors no Apache-licensed source, so none is required and none is added —
recorded so it reads as a decision, not an omission. Distinct from R-L3, which concerns code bundled
*into* our artifact.

**D2 (`0.5.0`) is superseded.** The repo shipped `0.5.0`, `0.6.0` and `0.7.0` while this phase sat
unbuilt. The launch version is whatever the release gate sets (`pnpm release`, `RELEASING.md`), not a
number pinned here. Read `0.5.0` throughout as `<LAUNCH_VERSION>`.

**Also drifted since approval** (mechanical, no decision needed): `files` now includes `skills/` — the
"consider bundling `skills/`" question is **answered and done**; `package.json` gained a `funding`
field; and the repo moved to a `dev` → `main` branching model, so this phase's "merge to `main`" reads
as "merge to `dev`, promote via the release flow."
...
**Added 2026-09-06 (§3.7):** the licensing sweep returns only superseded-annotated MIT references;
`LICENSE`/`package.json`/README/CONTRIBUTING agree on Apache-2.0 + DCO + no CLA; the DCO check exists
and is scoped to the PR range; `THIRD-PARTY-NOTICES.md` is generated, shipped, and whitelisted. The
DCO check **passing on a real PR** is operator-observed (§6), like the workflows themselves.
...
## 7. Decisions (LOCKED by Diego 2026-06-10 — all four to the recommended default; D2 superseded 2026-09-06, see §3.7)
